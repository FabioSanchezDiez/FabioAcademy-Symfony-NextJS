...
class EmailService
{
    ...
    {
        $email = (new Email())
            ->from('accounts@fabiocode.com')
            ->to($email)
            ->subject('Confirma tu cuenta en FabioCode Academy')
    ...
    private function createBody(string $name, string $token): string{
        ...
        return "<html>\n" .
            ...
            "    <a href='"."http://127.0.0.1:5500/verification.html?token=". $token ."'><button>Verificar</button></a>\n" .
